...
class Ship extends Phaser.Scene {
    constructor() {
        super("ship");
        this.location_x = 136; //캐릭터 스폰 위치
        this.location_y = 880;
    }
    ...
    preload () 
    {
        this.load.image('map5', 'assets/선착장.png');
        this.load.image('ground5', 'assets/ground_notsee.png');
        this.load.spritesheet('potal', 'assets/Potal.png',{ frameWidth: 89 , frameHeight: 240  });
       
        this.load.spritesheet('admin8', 'assets/선착장npc.png',{ frameWidth: 59, frameHeight:85});
    }
    
    create (){
        this.physics.world.setBounds( 0, 0, 1320 , 1355);  
        this.facing = 'left';
        this.cameras.main.setBounds(0, 0, 1300, 1355);
    ...
    // 이미지 설정 
        this.add.image (400, 400, 'map5');  //맵 이미지 (가로위치,세로위치,url)
                        

    
        this.platformsG.create(650, 1045, 'ground5').setScale(7).refreshBody(); // 가장 밑바닥 땅 
        this.platformsB.create(620, 602, 'ground5').setScale(1.7).refreshBody(); //공중 땅 블럭1
        this.platformsB.create(600, 840, 'ground5').setScale(1.5).refreshBody();//공중 땅 블럭2
        this.platformsB.create(860, 710, 'ground5').setScale(1.5).refreshBody();//공중 땅 블럭3
    
        this.player = this.physics.add.sprite(this.location_x, this.location_y, 'dude1');
        this.potal = this.physics.add.sprite(130,805,'potal').setImmovable(); //포탈 -setImmovable() -> 충돌에도 움직이지 않음 
        this.admin8 = this.physics.add.sprite(900,642, 'admin8').setInteractive(); // 운영자 -setInteractive() -> 이벤트 설정 가능메서드   
        ...
        this.physics.add.collider(this.potal, this.platforms); //ground블럭과 충돌여부 
         ...
         //운영자 캐릭터 설정-------------------
         this.admin8.body.setGravityY(-300) //운영자 중력설정
         this.physics.add.collider(this.admin8, this.platformsG); //ground블럭과 충돌여부 
         this.physics.add.collider(this.admin8, this.platformsB); //ground블럭과 충돌여부 
         this.admin8.setBounce(0); //땅에 닿았을때 다시 튀어오르는 정도
         this.admin8.setCollideWorldBounds(true); //맵 밖으로 안나가게 
     
          //운영자 움직임--------------
        this.anims.create({ 
            key: 'admin8',
            frames: this.anims.generateFrameNumbers('admin8',{start:0,end:3}), //프레임 선택  
        ...
        this.anims.create({ 
            ...
            repeat: -1  //반복
                ...
                this.admin8.anims.play('admin8',true); //운영자 스프라이트 움직임 실행
...
this.element = document.getElementById('input-box3');


//대화 켜기 (캐릭터 선택)
this.admin8.on('pointerdown',()=>{
    this.pause = true;
    console.log("클릭");
...
this.cameras.main.setFollowOffset( 0, 120);
    ...
    if(this.player.x >= (this.potal.x-40) && this.player.x <= (this.potal.x+40) && this.player.y >=(this.potal.y+30) && this.player.y <=(this.potal.y+130) ){ 
        if(this.cursors.up.isDown ){
            ...
            this.scene.start('hene', {x: 5669, y: -250});
    ...
    if(this.ctrl.isDown) {
        ...
        {if(this.pause)
            ...
            if(this.player.body.touching.down)
                this.player.setVelocityY(-600); //y축 속도 설정    
        }
