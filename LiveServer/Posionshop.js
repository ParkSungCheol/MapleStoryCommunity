...
class Posionshop extends Phaser.Scene {
    constructor() {
        super("posion");
        this.location_x = 137; //캐릭터 스폰 위치
        this.location_y = 480;
    ...
    preload () 
    {
        this.load.image('map4', 'assets/잡화상점.png');
        this.load.image('ground3', 'assets/ground_notsee.png');
        this.load.spritesheet('potal', 'assets/Potal.png',{ frameWidth: 89 , frameHeight: 240  });
      
        this.load.spritesheet('admin7', 'assets/잡화상점npc.png',{ frameWidth: 57, frameHeight:70});
    }
    ...
    create (){
        ...
        this.facing = 'left';
        this.cameras.main.setBounds(0, 100, 600, 600);
    ...
    // 이미지 설정 
        this.add.image (400, 400, 'map4');  //맵 이미지 (가로위치,세로위치,url)
    
        
    
        this.platformsG.create(400, 660, 'ground3').setScale(4).refreshBody(); // 가장 밑바닥 땅 
        this.platformsB.create(140, 540, 'ground3').setScale(0.4).refreshBody(); //공중 땅 블럭1
        this.platformsB.create(600, 370, 'ground3');//공중 땅 블럭2
        this.platformsB.create(690, 535, 'ground3').setScale(0.5).refreshBody();//공중 땅 블럭3
    
        this.player = this.physics.add.sprite(this.location_x, this.location_y, 'dude1');
        this.potal = this.physics.add.sprite(130,400,'potal').setImmovable(); //포탈 -setImmovable() -> 충돌에도 움직이지 않음 
        this.admin7 = this.physics.add.sprite(490,555, 'admin7').setInteractive(); // 운영자 -setInteractive() -> 이벤트 설정 가능메서드   
        ...
        this.physics.add.collider(this.potal, this.platforms); //ground블럭과 충돌여부 
         ...
         //운영자 캐릭터 설정-------------------
         this.admin7.body.setGravityY(-300) //운영자 중력설정
         this.physics.add.collider(this.admin7, this.platforms); //ground블럭과 충돌여부 
         this.admin7.setBounce(0); //땅에 닿았을때 다시 튀어오르는 정도
         this.admin7.setCollideWorldBounds(true); //맵 밖으로 안나가게 
     
          //운영자 움직임--------------
        this.anims.create({ 
            key: 'admin7',
            frames: this.anims.generateFrameNumbers('admin7',{start:0,end:17}), //프레임 선택  
        ...
        this.anims.create({ 
            ...
            repeat: -1  //반복
                ...
                this.admin7.anims.play('admin7',true); //운영자 스프라이트 움직임 실행
...
this.cameras.main.setFollowOffset( 0, 120);
    ...
    if(this.player.x >= (this.potal.x-40) && this.player.x <= (this.potal.x+40) && this.player.y >=(this.potal.y+30) && this.player.y <=(this.potal.y+130) ){ 
        if(this.cursors.up.isDown ){
            ...
            this.scene.start('hene', {x: 3709, y: 135});
