...
class Market extends Phaser.Scene {
    ...
    preload () 
    {
        this.load.image('map2', 'assets/자유시장.png');
        this.load.image('ground1', 'assets/ground_notsee.png');
        this.load.image('grounds', 'assets/ground_small_notsee.png');

     

        this.load.spritesheet('potal', 'assets/potal.png',{ frameWidth: 89 , frameHeight: 240  });    
        this.load.spritesheet('mesonpc', 'assets/자유시장npc.png',{ frameWidth: 156, frameHeight:91});
    ...
    create (){
        ...
        this.facing = 'left';
        this.physics.world.setBounds( 0, 0, 2769, 777);  
        this.cameras.main.setBounds(0, 0, 2769, 777);
    ...
    this.onLadder = false;
        ...
        this.add.image (2769/2, 777/2, 'map2');  //맵 이미지 (가로위치,세로위치,url)
        ...
        this.platformsG.create(800, 720, 'ground1').setScale(8).refreshBody(); // 가장 밑바닥 땅 
        this.platformsG.create(2000, 720, 'ground1').setScale(8).refreshBody();

        this.platformsB.create(260, 390, 'grounds').setScale(3).refreshBody(); //공중 땅 블럭1
        this.platformsB.create(300, 240, 'grounds').setScale(3).refreshBody();//공중 땅 블럭2

        //오른쪽 계단
        this.platformsB.create(1900, 330, 'grounds').setScale(8).refreshBody();    
        this.platformsB.create(2200, 330, 'grounds').setScale(8).refreshBody();    
        this.platformsB.create(1000, 406, 'grounds').setScale(2).refreshBody();
        ...
        this.potal = this.physics.add.sprite(660,469,'potal').setImmovable(); //포탈 -setImmovable() -> 충돌에도 움직이지 않음 
        this.mesonpc = this.physics.add.sprite(1000,350, 'mesonpc').setInteractive(); // 운영자 -setInteractive() -> 이벤트 설정 가능메서드   
        ...
        this.physics.add.collider(this.player, this.platformsG);    
        ...
        this.physics.add.collider(this.potal, this.platformsB); //ground블럭과 충돌여부 
    
         //npc 캐릭터 설정-------------------
         this.mesonpc.body.setGravityY(-300) //운영자 중력설정
         this.physics.add.collider(this.mesonpc, this.platformsB); //ground블럭과 충돌여부 
         this.mesonpc.setBounce(0); //땅에 닿았을때 다시 튀어오르는 정도
         this.mesonpc.setCollideWorldBounds(true); //맵 밖으로 안나가게 
     
          //운영자 움직임--------------
        this.anims.create({ 
            key: 'mesonpc',
            frames: this.anims.generateFrameNumbers('mesonpc',{start:0,end:3}), //프레임 선택  
        ...
        this.anims.create({ 
            ...
            repeat: -1  //반복
                ...
                this.mesonpc.anims.play('mesonpc',true); //운영자 스프라이트 움직임 실행
...
this.anims.create({ 
    ...
    this.element = document.getElementById('input-box1');
    
    
    //대화 켜기 (캐릭터 선택)
    this.mesonpc.on('pointerdown',()=>{
        console.log("클릭");
        this.pause = true;
    ...
    if(this.player.x >= (this.potal.x-40) && this.player.x <= (this.potal.x+40) && this.player.y >=(this.potal.y+30) && this.player.y <=(this.potal.y+88) ){ 
        if(this.cursors.up.isDown ){
          ...
          this.scene.start('hene', {x: 2470, y: 500});
