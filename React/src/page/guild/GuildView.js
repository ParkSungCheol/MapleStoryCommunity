...
const GuildView = ({ match, history }) => {
  ...
  console.log("길드는  ->" + guild)
  //조회수 올리기 
  const api = axios.create({
    baseURL: "http://localhost:8088"
  })
  api.post("/freeupdate/" + bnum, null, {
    params: {
      BNUMBER: bnum,
      BCLICK: click,
      BCLASS: bclass,
    ...
    <>
      ...
      <div className="free-view-wrapper">
            ...
            <>
              ...
              <div className="free-view-row">
                ...
                <table>
                  ...
                  <tr>
                    <td><label>길드</label></td>
                    <td><label>{data.GUILD}</label></td>
                  </tr>
                  <tr>
        ...
        <div id="freeview_btns">
          ...
          <button id="freeview_crud_btn" onClick={() => document.location.href = "http://localhost:3000/guild?" + urlparams}>목록</button>

          {(function() {
            if (userName === data.userid) return (<div> <button id="freeview_crud_btn" onClick={FreeDelete}>삭제</button>
            <button id="freeview_crud_btn" onClick={() => document.location.href = "http://localhost:3000/GuildUpdate/"+ no+"?"+urlparams}>수정</button></div>)
